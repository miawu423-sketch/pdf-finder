...
var results=[],retried=false;
var SKIP_MOD_HOSTS=['toutiao.com','toutiaocdn.com','toutiaoimg.com','baijiahao.baidu.com','mbd.baidu.com','dy.163.com','yidianzixun.com','k.sina.cn','k.sina.com.cn','ifeng.com'];
function skipMod(){var h=location.hostname.replace(/^www\./,'');return SKIP_MOD_HOSTS.some(function(d){return h.indexOf(d)!==-1;});}
var skipModified=skipMod();

// ====== Time utils — ALL outputs are Beijing Time ======
function pad(n){return n<10?'0'+n:''+n;}
function nowYear(){return new Date().getFullYear();}
function looksLikeDate(str){return /(?:20|19)\d{2}[-\/年\.]\d{1,2}[-\/月\.]\d{1,2}/.test(str)||/\d{4}[-\/\.]\d{1,2}[-\/\.]\d{1,2}\s+\d{1,2}:\d{2}/.test(str);}

// Direct formatting: parse components from string, output as Beijing Time
function bjFromString(str){
...
// Epoch ms → Beijing Time (UTC+8)
function bjFromEpoch(ms){
  var o=8*60, d=new Date(ms), lo=d.getTimezoneOffset();
  var bt=new Date(d.getTime()+(o+lo)*60000);
  return bt.getFullYear()+'/'+pad(bt.getMonth()+1)+'/'+pad(bt.getDate())+' '+pad(bt.getHours())+':'+pad(bt.getMinutes())+':'+pad(bt.getSeconds());
}

function dedup(){
  var seen={},u=[];
  results.forEach(function(r){var k=r.label+'|'+r.raw;if(!seen[k]){seen[k]=1;u.push(r);}});
  results=u;
}
...
// ====== parseTime — unified entry, returns Beijing Time string or null ======
function parseTime(str){
  if(!str||str.length>50)return null;str=str.trim();
  // ASP.NET /Date(ms+tz)/
  var am=/^\/Date\((\d{10,13})([+-]\d{4})?\)\/$/;
  var ama=am.exec(str);
  if(ama){var ts=+ama[1];if(ama[1].length===10)ts*=1000;return bjFromEpoch(ts);}
  // RFC 2822: Sun, 09 Mar 2025 22:21:38 +0900 (has timezone info)
  var rm=/^[A-Z][a-z]{2},\s/;
  if(rm.test(str)){var d2=new Date(str);if(!isNaN(d2.getTime()))return bjFromEpoch(d2.getTime());}
  // 2-digit year
  ...
  // Xinhua: 202605/2313:03:30
  var xm=str.match(/^(\d{4})(\d{2})\/(\d{2})(\d{2}:\d{2}:\d{2})$/);
  if(xm)str=xm[1]+'-'+xm[2]+'-'+xm[3]+' '+xm[4];
  // Timestamps (10 or 13 digits)
  if(/^\d{10}$/.test(str))return bjFromEpoch(+str*1000);
  if(/^\d{13}$/.test(str))return bjFromEpoch(+str);
  // No-year MM-DD HH:MM
...
function extractMeta(){
  ...
  document.querySelectorAll('meta[name][content]').forEach(function(el){
    ...
    if(n&&c&&/date|time|publish|pubdate|created|modified/i.test(n)&&looksLikeDate(c)){addResult('meta',n,c);}
  });
}
...
function extractJsonLd(){
  ...
  function walkJsonLd(obj){
    ...
    if(!skipModified){['dateModified','lastModified','modifiedDate'].forEach(function(f){if(obj[f])addResult('json-ld','modified:'+f,String(obj[f]));});}
    ['datePublished','publishDate','dateCreated','uploadDate'].forEach(function(f){if(obj[f])addResult('json-ld','published:'+f,String(obj[f]));});
...
function extractScriptVars(text){
  ...
  while((m=re.exec(text))!==null){
    ...
    if(!isNaN(dt.getTime())&&dt.getFullYear()>=2000&&dt.getFullYear()<=2100){
      ...
      addResult('script',vm?vm[1]:'inline /Date/',m[0],bjFromEpoch(ms));
    }
  }
  if(!skipModified){
  ...
  while((tm=tr.exec(text))!==null){
    ...
    var d2=new Date(v);if(!isNaN(d2.getTime())&&d2.getFullYear()>=2000&&d2.getFullYear()<=2100){addResult('script','timestamp:'+tm[1],tm[2],bjFromEpoch(v));}
  }
}

// ====== Layer 4: Time tags ======
function extractTimeTags(){
  document.querySelectorAll('time[datetime]').forEach(function(t){var dt=t.getAttribute('datetime');if(dt)addResult('time-tag','<time datetime>',dt);});
}
...
function extractAll(){
  ...
  extractTimeTags();extractDataAttrs();extractClassId();extractRegexBody();
  dedup();
}
extractAll();

// ====== Sort: precision first, then source reliability ======
...
function renderPanel(){
  var bi=getBestIdx(),h='<style>'+css+'</style><div class="P"><div class="H"><h2>发布时间提取 \u2014 '+
    (results.length?'找到 '+results.length+' 个':'未找到')+'</h2><button class="X" id="cls">\u00d7</button></div><div class="B">';
  if(!results.length){
...
var bi=getBestIdx();
if(bi>=0&&results[bi]&&results[bi].parsed){navigator.clipboard.writeText(results[bi].parsed).catch(function(){});}
if(results.length===0){
  var rc=0;
